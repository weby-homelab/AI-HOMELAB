...
> [!NOTE]
> Станом на **29 червня 2026 року (29.06.2026)**, Фазу 1 (Фундамент) повністю завершено із випередженням графіку! Проєкт активно працює над реалізацією Фази 2. Проведено апаратні бенчмарки на базі моделей Gemma 4 26B (MoE) та Ornith 1.0 35B (MoE, MTP від американської DeepReinforce Team) на робочій станції WS. Паралельно знято обмеження на локальне використання та порівняння передових китайських моделей (таких як Qwen, DeepSeek) для кодування.
