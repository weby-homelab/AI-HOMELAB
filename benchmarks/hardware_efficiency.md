...
### Gemma 4 26B (Q4_K_M) — Нове покоління (MoE)
...
Під час блекауту ключовою є не пікова швидкість генерації, а кількість згенерованих токенів на один ватт-годину ємності вашого акумулятора. Для досягнення максимальної автономності використовуйте наступні налаштування:

### 1. Вибір моделей для обмеженого живлення
*   **Ультра-енергоефективні (споживання системи до 10-15W):** 
    *   **Gemma 4 E2B / E4B** або **Phi-4 mini 3.8B**.
    *   Квантизація: **`Q3_K_L`** або **`IQ3_XS`** (3-бітна квантизація суттєво знижує навантаження на шину пам'яті, що дає суттєву економію енергії).
*   **Оптимальні для балансу (споживання системи до 25W):** 
    *   **Gemma 3 12B** або **Phi-4 Multimodal 5.6B** (з підтримкою аудіо/зображень).
    *   Квантизація: **`Q4_K_M`**.

### 2. Енергозбереження на рівні ПЗ (Inference Tuning)
*   **Миттєве вивантаження моделей (Standby Tax):** Звичайний режим очікування моделі у VRAM споживає 15-30W на GPU. Встановіть параметр keep-alive у мінімальне значення, щоб змусити GPU переходити в глибокий режим енергозбереження відразу після відповіді:
    ```bash
    export OLLAMA_KEEP_ALIVE=10s
    ```
*   **Жорсткі ліміти довжини генерації:** Скорочення `max_tokens` (наприклад, до 150) запобігає зацикленню чи довгим просторим відповідям, що прямо зберігає заряд батареї.
*   **Обмеження паралельності:** Використовуйте `OLLAMA_NUM_PARALLEL=1`, щоб уникнути одночасного розігріву кількох ядер процесора/відеокарти.

### 3. Налаштування апаратного ліміту (Nvidia GPU)
Обов'язково обмежуйте Power Limit перед початком інференсу. Це зрізає пікові стрибки споживання без значного падіння швидкості:
```bash
# Обмеження RTX 3060 до 80W (стандартно 170W)
sudo nvidia-smi -pl 80

# Обмеження RTX 4060 Ti до 90W (стандартно 165W)
