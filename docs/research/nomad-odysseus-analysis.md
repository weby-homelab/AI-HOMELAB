...
| :--- | :--- | :--- | :--- |
| **Основна мета** | Офлайн-сервер знань, карт та базового ШІ для виживання/навчання. | Локальний мультиагентний ШІ-робочий простір (Workspace). | Навчальний хаб, оптимізація заліза, безпека та стійкість до відключень світла (енергоавтономність) в Україні. |
...
2. **Проблема HTML-парсеру та блокування:** При виклику резервного HTML-парсеру (`_html_fallback`), DuckDuckGo часто блокує автоматичні запити та повертає сторінку з капчею (anomaly modal, HTTP 400), що призводить до отримання 0 результатів пошуку. Це спричинило помилкову відповідь моделі про відсутність репозиторію `antigravity-cli-statusline` на GitHub (запит у сесії `411f2dd2-28b6-4fa1-a73c-6df9e8215863`).
3. **Вирішення через бібліотеку `ddgs`:** 
   * У віртуальне середовище `/root/odysseus/venv` було встановлено пакет `ddgs` (офіційно перейменована версія `duckduckgo-search`), яка використовує JSON API замість парсингу HTML.
   * Конфігурація Odysseus (`services/search/providers.py`) нативно підтримує імпорт `from ddgs import DDGS`, тому після встановлення пакету та перезапуску сервісу `odysseus-ui.service`, пошук почав працювати безпосередньо через API без блокувань.
   * Успішно протестовано працездатність пошуку та оновлено некоректний запис у базі даних SQLite для сесії `411f2dd2-28b6-4fa1-a73c-6df9e8215863` на вичерпну та правильну інформацію про репозиторій **[AndyAWD/antigravity-cli-statusline](https://github.com/AndyAWD/antigravity-cli-statusline)**.

> [!TIP]
> **Рекомендація з оптимізації:** Для усунення помилок у логах `odysseus-ui` слід змінити порт `llama-server` (наприклад, на `8081` або `8090`) або перенести SearXNG на інший порт, щоб уникнути конфликту на `8080`. Також обов'язковим є встановлення бібліотеки `ddgs` у `requirements.txt` проєкту для стабільної роботи веб-пошуку.

---

> [!NOTE]
> Детальний практичний звіт про налаштування, тюнінг та вирішення проблем у роботі з Odysseus, OpenCode та Gemma 4 на робочій станції WS див. у файлі [**Практичний досвід роботи з локальним ШІ**](./local-ai-experience-june-2026.md).
