...
Цей модуль демонструє, як надати локальному ШІ-агенту можливість:
  ...
  3. Використовувати фолбек на текстовий пошук (якщо Ollama офлайн).
...
class MemoryItem(BaseModel):
    ...
    timestamp: str = Field(default_factory=lambda: datetime.now().isoformat(), description="ISO мітка часу створення")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Додаткові метадані")
    embedding: Optional[List[float]] = Field(None, description="Векторне представлення (якщо увімкнено)")
...
class AgentMemoryStore:
    ...
    def __init__(self, db_path: str = "agent_memory.db", ollama_url: str = "http://localhost:11434"):
    ...
    def _check_ollama(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning("Ollama returned status %s. Fallback to text search.", response.status_code)
        except Exception:
            logger.warning("Local Ollama not detected at %s. Vector embeddings disabled. Using keyword search fallback.", self.ollama_url)
    ...
    def add_memory(self, category: str, content: str, importance: int = 5, metadata: Optional[Dict[str, Any]] = None) -> MemoryItem:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (item.category, item.content, item.importance, item.timestamp, json.dumps(item.metadata), embedding_str)
            )
            item.id = cursor.lastrowid
            conn.commit()
            
        logger.info("Added memory ID %s [%s]: '%s'", item.id, item.category, item.content[:40] + "...")
        return item
    ...
    def update_memory(self, memory_id: int, content: str, importance: Optional[int] = None) -> bool:
    ...
    def search_memories(self, query_text: str, limit: int = 3) -> List[Tuple[MemoryItem, float]]:
        ...
        if query_vector and self.embeddings_enabled:
            ...
            # Фолбек на пошук слів в Python для гнучкості та надійності
...
if __name__ == "__main__":
    ...
    if all_memories:
        ...
        if first_id:
            ...
            store.update_memory(first_id, "Користувач любить темні інтерфейси OLED Black і термінальну верстку Ink.", importance=9)
            
            # Перевіряємо оновлення
            updated = store.get_all_memories(category="user_pref")[0]
            print(f"  Оновлений спогад [{updated.id}]: '{updated.content}' (важливість: {updated.importance})")
